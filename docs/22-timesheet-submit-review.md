...
| 状态 | 条件 | 文案 | 确认按钮 |
| --- | --- | --- | --- |
| `pending` | 核对进行中（实施时 `queued`/`reviewing` 合并为一个状态，见文末） | 有回合在跑时「等待当前对话完成后核对…」，否则「正在按企业工时流程要求核对…」，5 秒后追加已用时 | 禁用 |
...
- **`queued` 与 `reviewing` 未分开**：实现只有一个 `pending`，并用 `waiting` 记录开始时是否有回合在跑（文案相应不同）。分开需要订阅 agent 状态，当前没有接。
