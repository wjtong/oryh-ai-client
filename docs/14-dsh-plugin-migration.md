...
⚠️ **本节记录一次我自己的误判。** 写上面那段时，我看到 DSH 树里除 `analyzer.ts` 外还有 `tests/remote-model.spec.ts` 被修改，就断定"补丁漏了这个测试、可恢复性仍不完整"，并动手重新导出补丁。重新导出后的内容与已提交版本**逐字相同**——补丁**一直**覆盖着两个文件。我只 `head -20` 看了补丁开头（那里只有 analyzer 的 hunk）就下了结论，没有看完整个文件。这与本会话前面"断定补丁文件从来不存在"是同一类错误：凭推断代替核对，而核对只需要一条命令。

真正存在的（小）问题是基线里的 `target` 只写了 `analyzer.ts`，而守卫的重新导出提示用的正是这个字段——照着它导出会**悄悄丢掉那个回归测试**。已改为数组并在提示里 join，附注说明那个 spec 携带了本修复的回归测试（含"结构相似的无关包不得被识别为协议"这一反向用例）。

### 已决策：领域保持为库（评审 §4）

评审要求先做产品决策再决定是否继续抽包。核查代码确认了它的判断：领域现在是**编译期模块，不是运行时单元**——`createLocalOryhRuntime()` 一次性造出全部服务，44 个 `@Remote` 方法在同一个类上，16 个工具都经 `chat.install()` 注册，各领域共用一个 `OryhClientHost` 和同一个 fetch 生命周期。

核查中也澄清了一点：评审问的"某企业只启用库存、不启用工时"实际混了三种诉求，其中**按用户权限裁剪已经实现**（`canAccessPage` 管入口，`requirePage`/`requirePermission` 在服务层拦截，且有撤权后 Remote 被拒的测试）。真正缺失的是"按**企业**启停模块"，而它是与用户权限正交的第二个轴。

用户选择**整体交付**。已记为 [ADR-0008](adr/0008-domain-libraries-not-independent-plugins.md)，并在决策表新增 D-027、D-028：

- 领域保持为编译期库，不继续机械拆包，不以包数量作为完成指标。
- **不抽** `@oryh/dsh-connections` / `@oryh/dsh-workbench`；不引入动态领域注册框架。
- 菜单可见性只是展示，授权始终在服务层；将来若有按租户的模块启停，必须是独立的第二个轴，不并入 `PAGES`。

省下的最贵一块是 Remote 拆分：按领域拆会改变生成产物与客户端命名空间，而 typert 生成按包进行，领域越多生成面越大——这与尚未上游的外部 Remote 补丁叠加会放大失效暴露面。ADR 里写明了重新评估条件，其中一条正是"补丁进入上游后按包生成的成本下降"。

**至此评审四项全部收口**：§1 命令流通知丢失（已修，含证伪测试）、§2 异常分类与绑定生命周期（已用 `$stream` 重写）、§3 工时项目选择（已修，八次回放零复现）、§4 领域边界（已决策）。§5 的两条也已完成：值引用限制已更正并实测，补丁守卫已改为检查执行中的产物。

仍未做：§2 的 `rejected` 状态恢复入口 UI（交互决策）、干净克隆的冷启动验证。
