...
6. 当前结论依据了哪个 Skill、Policy、Workflow 或对象定义版本。
...
企业层（一次只激活一个；模块由能力动态生成）
├── 基础工作空间
│   ├── 我的工作
│   ├── 提交中心
│   ├── 决策中心
│   └── 业务线程
├── 领域工作空间（按 capability + eligible Skill 出现）
│   ├── 销售 / 采购 / 财务
│   ├── 人员与规则 / 自定义业务
│   └── 自动化与治理
├── 业务视图
...
2026-09-08 更新：下面的早期两栏草图已由 [三栏工作台设计](13-three-pane-interaction-design.md) 替代。当前实现采用左菜单、中间传统业务页面、右侧上下文助手；AI 会话服务尚待接入。

```text
┌────────────────────────────────────────────────────────────────────┐
│ ORYH  [上海示例公司 ▾]                 已连接  帮助  设置          │
├───────────────────┬────────────────────────────────────────────────┤
│ 我的工作          │ 上海示例公司 · 费用报销业务线程                │
│ 提交中心          │────────────────────────────────────────────────│
│ 决策中心  4       │ [费用申请] ─ [报销发票] ─ [付款] ─ [核销]     │
│ 业务线程          │     │审批事实 / 开放待办 / 当前阻塞项           │
│                   │                                                │
│ 销售              │ [证据包] 存储事实 · 服务端派生 · Agent 结论   │
│ 采购              │  总额、附件、适用政策、判断依据                 │
│ 财务              │  [打开详情] [刷新] [就此询问 AI]               │
│                   │                                                │
│ 会话              │                                                │
...
- “我的工作”“提交中心”“决策中心”“业务线程”是稳定信息架构，但内部动作仍按权限裁剪；领域工作空间按当前 capability、eligible Skill 和租户功能动态出现。
- 客户端不提供“切换角色”来解锁菜单。角色只是 capability 的来源之一，ORYH 服务端返回的当前权限才是执行依据。
- Skill audience 只控制业务指引投放。用户缺少某个 Skill 时可以查看 reach 解释，但界面不得建议通过增加角色来绕过业务分工。
- 权限或 Skill 投放变化后，当前写卡片立即失效，工作空间在下一次服务端校验后收敛；已经打开的页面显示“权限已变化”，不能继续使用旧工具目录。
...
`/todos?include=target` 返回的目标摘要用于首页首屏；进入处理前仍要读取实体详情、附件、approval facts 和适用规则。待办列表是当前持有人的事实源，通知只用于降低延迟，客户端在启动、恢复网络、完成动作和收到通知后都重新读取开放待办。
...
| 服务端派生卡 | 标明计算字段和读取时间 | outstanding、报价漂移、订单匹配、可用余额等服务端结果 |
| Agent 结论卡 | “依据 Skill/Policy”标记 | Agent 基于事实和规则作出的检查结论，不等同于服务端状态 |
...
- 结论卡提供“查看依据”，展示使用的 Skill、Policy、Workflow、对象定义版本和相关事实引用；依据变化后旧结论与 proposal 标记失效。
...
- 关键金额、币种、日期、对象或用户；
- 将写入的事实以及不可逆部分；
- 服务端当前状态、`updated_at`（若提供）和读取时间；ORYH 当前没有通用 ETag/version 时不得伪造版本保证；
- “确认执行”和“返回修改”两个清晰动作；
...
审批 Operation 以用户打开的 `todo_id` 为入口，重新读取并解析目标实体、轮次和序号。当前 ORYH 尚未提供把“校验 todo 归属、写 approval fact、完成 todo”合成单事务的通用端点，因此 UI 不得把客户端预检查描述成服务端原子保证。确认结果要分别显示“决定已记录”和“本人的待办已完成”；后者失败时保持部分成功。

### 8.3 业务线程与证据包

业务线程不是新的业务对象，而是从显式关系实时重建的阅读视图。例如报价到收款展示 quotation → sales order → sales invoice → receipt/payment → applications；采购到付款展示 purchase request → purchase order → receiving/inventory → purchase invoice → payment/applications。缺失的关系显示“尚未关联”，不能由 Agent 按标题、金额或日期猜测。

线程详情分四栏或四个清晰区块：

1. **单据事实：** 类型、编号、状态、明细、附件和更新时间；
2. **流程事实：** 已写入的 approval facts 与当前开放 todos；
3. **服务端派生：** outstanding、差异、匹配和可用余额；
4. **Agent 检查：** 使用的 Skill/Policy/Workflow、发现的问题和仍需人工决定的事项。

对象 status 不等于 workflow position。已通过节点从 approval facts 得出，当前持有人从开放 todos 得出，可能的下一步只来自 workflow 定义，并以“可能”而非“当前状态”展示。

### 8.4 费用报销与资金动作

费用线程按租户实际 workflow 展示直接付款或报销发票路径。发票路径使用 `reimbursement` 方向，并允许一个申请关联多个发票；客户端显示哪些费用明细已被开票，不能把未关联项静默视为已处理。

资金交互分成独立动作：记录付款、应用付款/核销、账户流水冲正。它们分别生成 R4 proposal 和确认卡，不提供“付款并自动核销”的模糊按钮，除非 ORYH 未来提供具有清晰事务语义的专用端点。已记账流水不在客户端直接编辑；更正通过服务端允许的 counter-entry 或专用纠正动作完成。
...
7. 销售、采购、财务等能力派生工作空间是否比固定角色菜单更容易理解；
8. 业务线程中的四类真相标记能否避免把 Agent 建议当成已执行结果。
