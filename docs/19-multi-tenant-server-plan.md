...
> **2026-09-14 修订：agent 成为主客户端，并查实凭据会进入模型请求与 Session。** 三件事改变了本方案：
>
> 1. **agent 是主客户端（[ADR-0010](adr/0010-agent-is-the-primary-client.md)）。** 保存、提交、审批、创建按 ORYH skill 在对话里确认后，由 agent 经 `bash` 执行；中间栏是辅助视图，页面上的「准备 → 确认 → 核对」只是其中一条路。确认、审计、崩溃恢复和试点管控里凡是默认“写入都经过客户端的确认记录”的地方都要改。
> 2. **ORYH 的 API key 会进入模型请求和 Session 记录。** ORYH 把个人 `api_key` 写在 SKILL.md 的正文里（本机实测 35 个技能中 34 个），而 Harness 的 `skill` 工具装载时把正文原样交给模型。于是 key 随每次装载发给模型接口，并随工具结果写进 Session。[ADR-0009](adr/0009-chat-pane-is-a-generic-oryh-agent.md) 与本文 §6.2 “凭据不进模型输入与 Session”的前提不成立。服务器版上这意味着 Session 存储本身就是凭据存储，共用的模型接口会收到所有租户的 key。**ORYH 提供不含凭据的技能包因此从 P1 升为服务器版 P0 前提**，见 §6.2 注记与 [S0-4](21-s0-acceptance.md)。
> 3. **中间栏跟随对话写入、用户菜单项、技能包持有人。** 对话写入后页面靠 `serverChange` 刷新，目前按 Session 发送；用户自建菜单项存在 Harness 存储域；技能包记录持有人。它们分别落到 §6.1、§7 与 P4。
>
> 各节的同日期注记是具体修改；另外更正了 §12 决策表里仍写着“Shell 首版全部关闭”的一行。
...
> **2026-09-14 修订。** agent 执行 skill 脚本（`bash`）期间同样不能回收：对话里的提交、审批就发生在这里，被杀掉的脚本可能已经把请求发给了 ORYH。回收判断要看 agent 的回合状态与正在运行的工具，而不只是模型推理。
...
| 业务草稿、意图、确认及核对状态 | 服务器事务存储适配器 | 保留版本、一次性确认和结果不明状态；只覆盖中间栏页面这条写入路径，对话里按 skill 的写入不经过这里（2026-09-14） |
...
| 技能包持有人记录 | 同目录 `.oryh-manifest.json` | 记录包属于哪个部署、租户、用户、员工；持有人变化即重装。每身份独立 runtime 与 skills 根，天然避开桌面版“一次只能以一个企业身份写入”的限制（2026-09-14） |
| Harness Session 记录 | 每身份隔离的持久目录 | **按凭据对待**：装载过带 key 的 skill 后，记录里就有该人的 `ORYH_API_KEY`（见 §6.2 注记）；在 ORYH 提供不含凭据的技能包之前，不进共享备份、诊断转储与导出，保留期限按凭据策略定（2026-09-14） |
| 用户自建菜单项 | 各 runtime 的 Harness 存储域（`DSH_HOME/storages/`），按 Workspace 与企业身份区分 | 每身份 runtime 即天然隔离；租户级菜单项需 ORYH 提供服务端资源，不在 AI Client 自建（[docs/24](24-user-menu-views.md)，2026-09-14） |
...
> **2026-09-14 修订：凭据不只在 skills 目录里，还进了模型请求和 Session。**
>
> 上面的例外假设 key 只躺在磁盘上。实测不是这样：ORYH 把 `api_key` 写在 SKILL.md 的**正文**（一个配置块里），Harness 的 `skill` 工具装载技能时把正文原样作为工具结果交给模型。因此：
>
> - **每次装载都把该人的 key 发给模型接口。** 共用模型接口的服务器版上，这个接口会收到所有租户、所有用户的 key；模型服务商因此要按“处理凭据”来审批，而不只是“处理业务数据”。
> - **Session 记录里有 key。** 工具结果随 Session 持久化，Session 的备份、导出、诊断、分享都会带出 key；“浏览器、Session、模型输入和普通日志不包含真实凭据”这一句在现状下不成立。
> - **不能靠清洗解决。** ADR-0002 已定不以正则清洗任意文本；即便遮住 SKILL.md，脚本（`.py` 里也有 key）被 agent 读出时同样会进入 Session。
>
> 结论：**ORYH 提供不含凭据的技能包是服务器版的 P0 前提**——技能正文与脚本都不含 key，脚本从每身份的凭据文件或凭据代理读取。在它落地之前，服务器版不进入 P1；S0 增加一项专门验证（[S0-4](21-s0-acceptance.md)）。桌面版暂不遮住 SKILL.md 里的 key（2026-09-14 决定），技能包仍按原样安装；即便以后遮盖，也不作为服务器版的解决方案。
...
> **2026-09-14 修订。** 上一段只覆盖中间栏页面的写入。对话里的写入由 agent 按 skill 执行，确认记在 Session 里，客户端没有对应的意图记录可供 reconcile。它的恢复语义由 skill 规定：重试一次已确认的写入之前，先回读服务端确认它尚未落地（ORYH 的提交接口本身幂等）。服务器版要保证的是：runtime 重启后 Session 历史可见，被中断的回合标为中断，agent 在新回合里先回读再行动——不由客户端替 agent 重发。
...
> **2026-09-14 修订（[ADR-0010](adr/0010-agent-is-the-primary-client.md)）。**
>
> - **确认发生在两处。** 页面这条路仍是一次性确认记录；对话这条路是 agent 按 skill 复述将要写入的事实、得到用户明确同意——确认内容在 Session 里。授权链的最后一环因此是“操作确认（页面确认或对话中的明确同意）”。
> - **审计分两层。** 页面写入有 operationId；对话写入由 skill 脚本直接调用 ORYH，客户端没有 operationId，权威审计是 ORYH 服务端按 API key 身份记录的操作，Session 里的工具调用是补充证据（且含凭据，见 §6.2 注记）。
> - **对话写入要反映到页面，而且要按身份，不按 Session。** 当前实现在跑过 `bash` 的回合结束时，只给该 Session 的页面发 `serverChange`；同一身份的其他标签页、其他 Session 不会刷新（桌面版切换会话时也有这个小缺口）。服务器版多标签页要把刷新信号的范围改为身份 runtime，见 P4。
...
| 模型额度归属 | 提交前规范核对要调用模型；额度耗尽会让该身份**无法提交**（强制模式，见 [提交前核对](22-timesheet-submit-review.md)）。共享额度下一个租户可以耗尽其他租户的提交能力，因此额度必须按身份或租户计量。2026-09-14 起对话里的一切写入也依赖模型（ADR-0010），额度耗尽即该身份只能在页面上查询与编辑 |
| 刷新信号范围 | `serverChange` 目前按 Session 发送；服务器版改为按身份 runtime 发送，使同一身份所有标签页在对话写入后都刷新（2026-09-14） |
| 不含凭据的技能包 | ORYH 侧交付；客户端随之把脚本的 key 来源接到每身份凭据文件或凭据代理。服务器版 P0 前提（2026-09-14，见 §6.2 注记） |
...
3. 实现业务意图/草稿的服务器存储适配器，保留 prepare/confirm/reconcile 状态机（只服务页面写入路径；对话写入不经过它，见 §6.3 注记）。
...
3. 完善命令流绑定代次、重连和单 Session 编辑者策略，覆盖多标签页；`serverChange` 改为按身份 runtime 发送，一个标签页里的对话写入让同身份所有页面刷新。
...
7. 回放对话里的提交与审批（按 skill 复述、明确同意后写入），核对 ORYH 记录的操作人是该身份、中间栏在回合结束后显示新状态，并验证技能包持有人与 runtime 身份不一致时 agent 不写入。

交付：完整可用的服务器版业务工作台及自动化回归。

退出条件：模型不可用时传统页面的查询与编辑仍可用，带流程定义的提交明确提示需要核对；A 的模型 Key 不用于 B 的任务；切换企业不改变另一标签页的请求身份；提交与审批只在用户明确同意（页面确认或对话中同意）后执行，并经过分配与状态校验；对话写入后同身份所有标签页显示服务端新状态。
...
2. 首先开放查询与未保存表单填写；完成验收后再按业务风险开放正式提交与审批。2026-09-14 注：agent 经 skill 直接写入，客户端已无法单方面“先不开放提交”；试点期的写入范围要靠 ORYH 侧的角色授权与技能包内容来收，并在试点记录里写明所用角色。
...
| A1 在对话里提交自己的工时 | ORYH 记录的操作人为 A1；写入前有复述与明确同意；A1 其他标签页随之显示已提交 |
| A1 runtime 的技能包属于别的身份（持有人记录不符） | agent 不写入，先重新同步技能包 |
| 装载带凭据的 skill 后检查模型请求与 Session 持久化 | 不出现任何 `ORYH_API_KEY`（现状不通过，依赖 ORYH 提供不含凭据的技能包，见 §6.2 注记） |
...
| Shell、代码与插件安装 | Shell 与 skill 脚本执行已开放（ADR-0009/0010），隔离靠 OS 身份与执行容器；插件安装仍关闭（2026-09-14 更正） | P0 |
| 不含凭据的技能包 | 由 ORYH 提供，服务器版 P0 前提；落地前不进入 P1 | P0（外部依赖，最早启动） |
| 对话写入的刷新范围 | 按身份 runtime 发送，同身份所有标签页刷新 | P4 前 |
...
服务器版完成的依据是：真实多 tenant、多用户在同一入口同时使用，浏览器自动进入自己工作空间；原生 Harness 和 ORYH 业务能力均落实归属校验；Chat 与传统页面一致，对话里的写入在同身份的页面上可见；正式写入保留用户确认（页面确认或对话中的明确同意）；模型请求与 Session 记录中不含 ORYH 凭据；重启、断网、撤权、并发编辑可预测；数据及密钥可恢复；部署和回滚有经过验证的路径。
