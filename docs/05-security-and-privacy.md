...
8. 服务端事实、服务端派生值、Agent 结论和未执行 proposal 不得在日志、模型上下文或 UI 中相互冒充。
...
| 业务线程投影与 evidence packet | Confidential/Restricted | 逐来源授权、有界字段、租户分区、依据版本、可重建 |
...
- 工作空间显隐由 capability 与 eligible Skill 派生，但显隐从不构成授权；每次 Operation 都由服务端重新判定。
- 业务线程只能合并同一 connection 下当前调用可见的事实；一个来源 404/不可见时显示“不存在或不可见”，不能从其他记录反推出隐藏对象。
...
- 用 A 的 typed link 或对象 id 构造业务线程时，projector 不访问 B，也不按标题/金额/日期建立推测关系。
...
ORYH 当前多数业务读取是租户范围，而 payroll 与 restricted policy 等另有专门限制。客户端不能把“我能看到的列表”误称为“只属于我的数据”，也不能以首页便利为理由请求更宽能力。附件正文优先经所属单据的 document-scoped 路由读取；工资、受限政策和资金账户使用专用工具，默认不进入通用搜索索引或跨域线程摘要。
...
3. 工具集合按 tenant、permission、eligible Skill audience 和风险策略收窄；
...
12. Agent 结论绑定 evidence digest 与 Skill/Policy/Workflow 版本；依据变化后不得沿用旧确认。
...
Skill audience 是内容投放，不是授权。客户端只有在 capability 满足且 Skill 采用 capability 模式或当前用户命中目标 audience 时才加载该 Skill；reach 响应中的 `granted_by_roles` 只能解释投放，不得触发角色申请、自动提权或建议管理员放宽权限。
...
| 付款/核销/账本 | 金额、币种、收付方向、账户、对象强确认；付款记录与 application 分开；要求服务端 step-up 后开放 |
...
确认依据同时固定当前详情/明细、服务端派生指标、approval facts、开放 todos 和适用规则版本。ORYH 当前没有通用 ETag/version，因此客户端使用可得的 `updated_at`、状态和执行前重读降低竞争窗口，但不能声称提供了服务端乐观锁。
...
当前通用审批 API 没有把 todo 归属校验、approval fact 与 todo completion 合成一个原子事务，也没有统一强制 no-self-approval。客户端必须从 `todo_id` 出发重新读取目标、轮次和序号，禁止 Agent 自填任意审批对象；但这只是纵深防御，正式企业保证仍需要 ORYH 后端专用原子端点或等价约束。客户端不得把 Skill 中的职责分离约定描述成数据库强制规则。

### 9.3 并发冲突

409 或版本不匹配后重新读取服务端事实，废弃原确认 proposal，向用户展示变化并重新确认。客户端不做 last-write-wins 隐式覆盖。

ORYH 当前结算写路径已经使用 PostgreSQL 行锁并具有真实并发测试，客户端把它作为服务端发布基线而不是未修复风险。新增付款、核销、账户流水或报销端点仍需分别通过同等级测试，并补齐 request-body 幂等摘要、统一 Decimal API 语义和结果未知恢复；现有局部修复不能外推为所有财务写入都安全。
...
- [ ] 工作空间 capability/audience 真值表、业务线程显式关系和局部不可见测试通过；
- [ ] evidence 或 Skill/Policy/Workflow 版本变化使旧 proposal 与确认失效；
...
- ORYH 当前结算并发问题已有行锁与真实 PostgreSQL 测试；新增财务路径、统一幂等/Decimal 语义以及 Hosted Flow Runner 隔离仍是独立服务端风险，客户端不能补造这些保证；
