...
export function Workbench({ page, connection, operations, onDirtyChange, settings, notices }: {
    ...
    useEffect(() => { if (page === 'timesheets') setTimesheetVisited(true); else if (page === 'timesheet-approvals') setApprovalVisited(true); else if (page !== 'settings') setVisited(current => current.includes(page) ? current : [...current, page]); }, [page]);
    ...
    <main ref={main} className="business-content">
      ...
      {visited.map(id => <section key={id} hidden={page !== id} aria-label={t("text19", { value0: pages[id].title })}>
        ...
        {id==='list-projects'&&<ProjectPanel connection={connection} active={page===id} {...(navigation?.target==='project'?{navigation}: {})} onDirtyChange={setProjectDirty} onContext={value=>report(id,value)}/>}
