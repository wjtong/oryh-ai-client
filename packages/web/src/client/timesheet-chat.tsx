...
export function TimesheetChat({connectionId,manager,fields,headerId,todoId,navigationId,navigationReady=true,localEdits,busy,dirty,onApply}:{connectionId:ConnectionId;manager:boolean;fields?:TimesheetFields;headerId?:string;todoId?:string;navigationId?:string;navigationReady?:boolean;localEdits:string;busy:boolean;dirty:boolean;onApply:(action:TimesheetAction)=>void}){
  ...
  useEffect(()=>{
    ...
    async function poll(){
      try{const p=await api.timesheetChatPoll({sessionId:sessionId!,connectionId,pageKey});if(live&&p&&p.revision===version.current.revision&&p.id!==seen.current){if(p.action.kind==='create'&&!isBusy.current){seen.current=p.id;apply.current(p.action);setMessage('Chat 已更新未保存表单，可继续描述要修改的内容。')}else setProposal(p)}}catch(e){if(live)setMessage(e instanceof Error?e.message:'Chat 同步失败');return}
      if(live)timer=setTimeout(()=>void poll(),350)
    }
    void api.timesheetChatSync(state).then(()=>{if(live){setMessage('描述工作内容即可自动填写右侧表单；保存、提交和审批仍需确认。');void poll()}}).catch(e=>{if(live)setMessage(e instanceof Error?e.message:'Chat 同步失败')})
