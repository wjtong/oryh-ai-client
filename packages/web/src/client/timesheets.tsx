...
export function TimesheetPanel({connection,manager,active,navigationId,navigation,onDirtyChange}:{connection:ConnectionSummary;manager:boolean;active:boolean;navigationId?:string;navigation?:ChatNavigation;onDirtyChange:(v:boolean)=>void}) {
  ...
  const filteredHeaders=headers.filter(r=>`${r.period_start} ${r.period_end} ${r.status} ${r.source_report_text}`.includes(query)&&(!status||r.status===status))
  ...
  const statusName=(value:string)=>({draft:'草稿',submitted:'待审批',approved:'已通过',rejected:'已拒绝',returned:'已退回'}[value]??value)
  ...
  return <section ref={panel} className="oryh-timesheets" aria-busy={busy}>
    <header className="business-page-header">{(editor||detail)&&<Button appearance="subtle" disabled={busy} onClick={()=>dirty?setLeaveOpen(true):returnToList()}>← 返回列表</Button>}<h1>{editor?(detail?'编辑工时':t('tsNew')):detail?`${detail.header.period_start} — ${detail.header.period_end}`:t(manager?'tsApprovals':'tsMine')}</h1>{dirty&&<span className="muted">请先保存或放弃当前修改</span>}{!editor&&!detail&&<div className="toolbar"><Button disabled={busy} onClick={()=>void run(refresh)}>{t('tsRefresh')}</Button>{!manager&&hasPermission(connection.identity,'timesheet.submit_own')&&<Button appearance="primary" disabled={busy||dirty} onClick={()=>{setFields(blank());setEditor(true);setDetail(undefined);setEditing(undefined);setDirty(true)}}>{t('tsNew')}</Button>}</div>}</header>
    ...
    {!editor&&!detail&&<section className="surface table-surface" aria-label={t(manager?'tsApprovals':'tsMine')}><div className="query-toolbar"><label>关键词<input aria-label={t('tsSearch')} placeholder="日期、工作说明或状态" value={query} onChange={e=>{setQuery(e.target.value);setPage(1)}}/></label>{!manager&&<label>状态<select value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">全部状态</option>{[...new Set(headers.map(h=>h.status))].map(s=><option key={s} value={s}>{statusName(s)}</option>)}</select></label>}{(query||status)&&<Button appearance="subtle" onClick={()=>{setQuery('');setStatus('');setPage(1)}}>清空条件</Button>}</div><div className="table-scroll"><table><thead><tr><th>{manager?'审批事项':'申报期间'}</th><th>工作说明</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{manager?visibleTodos.map(r=><tr key={r.id}><td><button className="record-link" disabled={busy} onClick={()=>void run(()=>open(r.entity_id,r.id))}>{r.title}</button></td><td>{r.description||'—'}</td><td><span className="record-status status-pending">待审批</span></td><td><Button appearance="subtle" size="small" disabled={busy} onClick={()=>void run(()=>open(r.entity_id,r.id))}>查看</Button></td></tr>):visibleHeaders.map(r=><tr key={r.id}><td><button className="record-link numeric" disabled={busy} onClick={()=>void run(()=>open(r.id))}>{r.period_start} — {r.period_end}</button></td><td>{r.source_report_text||'—'}</td><td><span className={`record-status ${r.status==='submitted'?'status-pending':'status-neutral'}`}>{statusName(r.status)}</span></td><td><Button appearance="subtle" size="small" disabled={busy} onClick={()=>void run(()=>open(r.id))}>打开</Button></td></tr>)}</tbody></table></div>{!count&&!busy&&<div className="empty-state"><h3>{query||status?'没有符合条件的记录':t('tsEmpty')}</h3><p>{query||status?'尝试更换关键词或清空筛选条件。':manager?'新的工时审批将在这里显示。':'新建工时单，或通过 Chat 描述你的工作。'}</p></div>}<footer className="table-footer"><span>共 {count} 条</span><div className="pagination"><Button size="small" disabled={busy||currentPage<=1} onClick={()=>setPage(currentPage-1)}>上一页</Button><span>{currentPage} / {pages}</span><Button size="small" disabled={busy||currentPage>=pages} onClick={()=>setPage(currentPage+1)}>下一页</Button></div></footer></section>}
    ...
    {detail&&!editor&&<div className="surface"><h2>{(detail.canEdit&&hasPermission(connection.identity,'timesheet.submit_own'))&&!manager?'编辑工时':'工时详情 · 只读'}</h2>{!(detail.canEdit&&hasPermission(connection.identity,'timesheet.submit_own'))&&<p>当前身份或单据状态不允许修改工时内容。</p>}<p>{t('tsEmployee')}：{detail.header.employee_id} · {t('tsStatus')}：{detail.header.status}</p><p>{detail.header.source_report_text}</p>{(detail.canEdit&&hasPermission(connection.identity,'timesheet.submit_own'))&&!manager?<p>{t('tsTotal')}：{sum(detail.entries)}</p>:renderLines(detail.entries)}
      <PreferenceDetails preferenceKey={`timesheets:${manager}:trailOpen`}><summary>{t('tsTrail')}</summary>{detail.approval_records.map(a=><p key={a.id}>{t('tsRound')} {a.round_no}/{a.sequence_no} · {a.action} · {a.approver_id} · {a.acted_at}<br/>{a.comment}</p>)}</PreferenceDetails>
