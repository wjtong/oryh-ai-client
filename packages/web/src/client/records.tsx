...
export function RecordPanel({kind,filterCommand,columns,onColumns,connectionId,onContext}:{filterCommand?:ChatNavigation;columns:string[];onColumns:(columns:string[])=>void;kind:RecordKind;connectionId:string;onContext:(value:PageContext)=>void}){
 ...
 const [data,setData]=useState<RecordPage>(),[selected,setSelected]=useState<BusinessRecord>(),[page,setPage]=useState(1),[query,setQuery]=useState(''),[search,setSearch]=useState(''),[reload,setReload]=useState(0),[busy,setBusy]=useState(true),[error,setError]=useState('')
 const [queryFields,setQueryFields]=useState<string[]>([]),[products,setProducts]=useState<ProductOption[]>([]),[appliedProducts,setAppliedProducts]=useState<ProductOption[]>([])
 ...
 return <section className="business-page oryh-records" aria-busy={busy}>
  ...
  {selected?<div className="surface document-detail"><h2>记录信息</h2><p className="data-caption">来源：ORYH · {data?.fetchedAt&&new Date(data.fetchedAt).toLocaleString()}</p><dl className="document-fields">{selected.fields.map(f=><div key={f.label}><dt>{f.label}</dt><dd>{f.value}</dd></div>)}</dl></div>:<>
   <form className="record-search" onSubmit={e=>{e.preventDefault();setPage(1);setSearch(query);setAppliedProducts(products);setReload(v=>v+1)}}><Field label={queryLabels[kind]}><Input value={query} onChange={(_,d)=>setQuery(d.value)} maxLength={200}/></Field>{kind==='inventory-item-details'&&queryFields.includes('product_code')&&<Field label="产品（多选）"><ProductPicker connectionId={connectionId} value={products} onChange={setProducts}/></Field>}<Button type="submit" disabled={busy}>查询</Button><Button type="button" onClick={()=>{setQuery('');setSearch('');setProducts([]);setAppliedProducts([]);setPage(1);setReload(v=>v+1)}}>清空条件</Button></form>
   {kind==='inventory-item-details'&&<details className="advanced-filters"><summary>查询字段</summary><label><input type="checkbox" checked={queryFields.includes('product_code')} onChange={e=>configureFields(e.target.checked?['product_code']:[])}/>产品</label></details>}
   ...
   {kind==='inventory-item-details'&&<p className="data-caption">库存流水为追加记录；默认查询活动库存项，指定库存项编号可查看其历史。</p>}
   {busy?<p role="status">正在读取…</p>:data&&<div className="surface table-surface"><div className="table-scroll"><table><thead><tr>{columns.map(c=><th key={c}>{catalog[c]}</th>)}</tr></thead><tbody>{data.rows.map(row=><tr key={row.id}>{columns.map((c,index)=><td key={c}>{index===0?<button className="record-link" onClick={()=>setSelected(row)}>{row.fields.find(f=>f.label===catalog[c])?.value??'—'}</button>:row.fields.find(f=>f.label===catalog[c])?.value??'—'}</td>)}</tr>)}</tbody></table></div>{!data.rows.length&&<p className="record-empty">没有符合条件的记录</p>}<footer className="table-footer"><span>共 {data.total} 条 · 当前 {data.rows.length} 条</span><div className="pagination"><Button size="small" disabled={page<=1} onClick={()=>setPage(v=>v-1)}>上一页</Button><span>{page} / {data.pages}</span><Button size="small" disabled={page>=data.pages} onClick={()=>setPage(v=>v+1)}>下一页</Button></div></footer></div>}
  </>}
 </section>
}
