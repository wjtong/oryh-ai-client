...
export function TodoChat({connectionId,todoId,visibleTodos=[],listContext='',navigationId,onOpen}:{connectionId:ConnectionId;todoId?:string;visibleTodos?:{id:string;title:string}[];listContext?:string;navigationId?:string;onOpen?:(id:string,navigationId:string)=>void}){
  ...
  useEffect(()=>{
    ...
    async function poll(){
      try{const n=await api.chatHomePoll({sessionId:sessionId!,connectionId});if(live&&n?.target==='todo'&&n.todoId&&n.listRevision===listRevision&&visibleTodos.some(t=>t.id===n.todoId)){openCallback.current?.(n.todoId,n.id);return}}catch{}
      if(live)timer=setTimeout(()=>void poll(),350)
    }
    void poll()
    return()=>{live=false;if(timer)clearTimeout(timer)}
  },[api,connectionId,sessionId,todoId,ready,listRevision])
