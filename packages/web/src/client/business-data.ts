...
export function businessRows(result: OryhOperationResult, t: BusinessText = defaultText): BusinessRow[] {
    if (result.operationId === 'list-projects')
        return (result.result.data as readonly OryhProject[]).map(row => ({ id: row.id, title: row.name, status: row.status, date: row.startDate ?? '', secondary: row.client ?? '', entityType: 'project', fields: [[t("text179"), row.code ?? '—'], [t("text76"), row.client ?? '—'], [t("text28"), row.startDate ?? '—'], [t("text180"), row.endDate ?? '—']] }));
