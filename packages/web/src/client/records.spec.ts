...
it('restores Chat query fields and products on remount, refetches data, and isolates identities',async()=>{
 ...
 const render=(scope:string,filterCommand?:any)=>root.render(h(PreferenceScope.Provider,{value:scope},h(RemoteContext.Provider,{value:{recordList} as never},h(RecordPanel,{kind:'inventory-item-details',connectionId:scope,columns:['id'],onColumns:()=>{},onContext,...(filterCommand?{filterCommand}:{})}))))
 await act(async()=>render('tenant-a:user-a',{id:'query',target:'filters',queryFields:['product_code'],products:[{id:'p',code:'PT-HEAD',name:'打印头'}],expiresAt:Date.now()+10000}))
