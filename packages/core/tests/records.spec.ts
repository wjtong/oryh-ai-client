...
describe('business record queries',()=>{
 ...
 it.each(['sales-orders','inventory-items','inventory-item-details','shipments'] as const)('uses the fixed %s read endpoint with encoded server paging',async kind=>{
  const request=vi.fn(async()=>body),service=new RecordService({request} as unknown as OryhHttpClient,async()=>connection,()=>connection)
 ...
 it('rejects arbitrary paths, invalid pages and changed tenant scopes',async()=>{
  const request=vi.fn(async()=>body),service=new RecordService({request} as unknown as OryhHttpClient,async()=>connection,()=>({...connection,origin:'https://other.invalid'}))
...
it('resolves ledger products once per position without exposing relation secrets',async()=>{
 ...
 const service=new RecordService({request} as unknown as OryhHttpClient,async()=>connection,()=>connection)
...
it('rejects invalid or cross-tenant product selection lookup',async()=>{
 ...
 const service=new RecordService({request} as unknown as OryhHttpClient,async()=>connection,()=>current)
