...
export class ProjectChat {
 ...
 constructor(private ctx:Context,private api:OryhProjectRemote|undefined,private identity:(session:string)=>string){}
 current(id:string){return this.states.get(id)}
 clear(id:string){this.states.delete(id);this.proposals.delete(id)}
 sync(s:ProjectChatState){if(this.identity(s.sessionId)!==s.connectionId)throw fail('企业已切换。');const old=this.states.get(s.sessionId);if(old?.pageKey===s.pageKey&&old.revision>s.revision)return;this.states.set(s.sessionId,structuredClone(s));if(old?.revision!==s.revision)this.proposals.delete(s.sessionId)}
 ...
 async fill(id:string,revision:number,input:unknown,signal:AbortSignal){
  ...
  try{const end=Date.now()+10000;while(Date.now()<end){signal.throwIfAborted();this.identity(id);const now=this.states.get(id);if(!now||now.pageKey!==s.pageKey)throw fail('项目页面已关闭。');if(now.revision>revision){if(JSON.stringify(now.fields)!==JSON.stringify(fields))throw fail('用户已修改表单，请重新读取。');return '右侧项目表单已更新，尚未创建，需用户核对确认。'}await new Promise(r=>setTimeout(r,100))}throw fail('填写未获页面确认，请重新读取。')}finally{if(this.proposals.get(id)===p)this.proposals.delete(id)}
 }
