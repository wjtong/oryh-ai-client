...
function fixture(){let page=true;const prepare=vi.fn(),confirm=vi.fn();const p=new ProjectChat({} as Context,{projectOptions:async()=>({canCreate:true}),projectPrepare:prepare,projectConfirm:confirm} as unknown as OryhProjectRemote,()=>{if(!page)throw Error('left');return 'c'});p.sync(state);return {p,prepare,confirm,leave:()=>{page=false}}}
describe('project chat proposals',()=>{
 it('only updates local fields and waits for UI acknowledgement',async()=>{const f=fixture(),next={...fields,project_name:'Updated'};const pending=f.p.fill('s',1,next,new AbortController().signal);const suggestion=f.p.poll({sessionId:'s',connectionId:'c' as never});expect(suggestion?.fields).toEqual(next);f.p.sync({...state,revision:2,fields:next});expect(await pending).toContain('尚未创建');expect(f.prepare).not.toHaveBeenCalled();expect(f.confirm).not.toHaveBeenCalled()})
 ...
 it('does not claim an update if the user changes the form first',async()=>{const f=fixture(),pending=f.p.fill('s',1,{...fields,client:'AI'},new AbortController().signal);f.p.sync({...state,revision:2,fields:{...fields,client:'Human'}});await expect(pending).rejects.toThrow(/用户已修改/);expect(f.p.poll({sessionId:'s',connectionId:'c' as never})).toBeUndefined()})
})
